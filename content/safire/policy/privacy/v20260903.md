---
date: 2026-09-03T12:25:00+02:00
slug: privacy-v20260903
tags:
  - policy
  - privacy
  - versioned
title: Privacy Statement v20260903
url: /safire/policy/privacy/v20260903.html
draft: true
---

> This is a update of the [v20251128]({{< relref "v20251128.md" >}}) version to reflect changes to how transfer notices are stored.
{.message-box}
...
SAFIRE processes the following information:
  ...
  * Every time you try and log in via the Federation Hub, we log a
    ...
    record your username (eduPersonPrincipalName) and affiliation
    information, although this is not routinely collected except in a
    partially redacted form. This information is collected to allow us
    to resolve security incidents and debug problems.

  * Our [Test Service Provider](https://testsp.safire.ac.za) allows you
    to verify the information sent from your identity provider, and
    receives all attributes. It evaluates these for compliance with
    various standards and baselines in order to confirm your identity
    provider is working correctly. It logs the results of these tests
    together with a small amount of personal information
    (based on the [REFEDS R&S bundle](https://refeds.org/category/research-and-scholarship))
    to allow Federation staff to assist with debugging of your home
    organisation with debugging their identity provider.
...
We set cookies in your browser recording various session
...
Our analytics partners may also set cookies.

## How we record which transfer notices you've seen

When we show you a list of attributes that we're sending to a service
provider, we keep a record of this. We do this by setting a cookie in
your browser or saving a record in our data store. If consent is
required, you can opt out by choosing the "only this time" option.

The record of a transfer notice records one-way [cryptographic hashes](https://simple.wikipedia.org/wiki/Cryptographic_hash_function) of the following:
  * your [eduPersonPrincipalName]({{< ref "/technical/attributes/edupersonprincipalname.md" >}}) or other subject identifier;
  * the service provider's entity ID; and
  * the attribute values you saw (this helps us know when to show the notice again)

It is not possible to retrieve the original personal information from this record.

You can ask us to delete these records at any time. Doing so will
[withdraw any consent]({{< ref "/users/help.md#changed-your-mind-about-sending-information-to-a-service-provider" >}}) you may have given.
...
Logs of authentications (including some partially redacted personal
information) are kept for a period of up-to one year (366 days).
Deidentified log files may be retained longer.

Transfer notice records expire after a maximum of one year.

The Test Service Provider retains the personal information logged
for debugging purposes for no more than one week (7 days).

Analytical data and anonymised statistics are currently retained indefinitely.

Where backups are made, these may include any of the logs or
personal information described here. Backups are stored on a
separate system with all data encrypted at rest. Decryption
keys are not stored on the same system. Such encrypted backups
may be retained for a further year after they are made.
